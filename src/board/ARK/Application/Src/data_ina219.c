...
#define INA219_COUNT 1

static ina219_t hina[INA219_COUNT] = { 0 };
static float volt[INA219_COUNT];
static float amp[INA219_COUNT];
...
void data_ina219_read() {
    for (int i = 0; i < INA219_COUNT; i++) {
        ...
        amp[i] = ina219_current_convert(&hina[i], sd.current);
        volt[i] = ina219_bus_voltage_convert(&hina[i], pd.busv);
    }
}
